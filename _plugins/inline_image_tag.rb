...
# Description: Create figure/img HTML blocks with optional classes and captions.
# This is a YAML-dependent Liquid tag plugin for Jekyll for those who fear link
# rot.
#
...
module Jekyll
  class FigureImageTag < Liquid::Tag
    ...
    def render(context)
      ...
      # The tags use indexes starting from one to make it easier for editors to understand.
      @array_index = Integer(@index) - 1

      @base_url = Liquid::Template.parse("{{ site.#{@image_url} }}").render(context)
      @src = Liquid::Template.parse("{{ page.images[#{@array_index}].url }}").render(context)
      @alt = Liquid::Template.parse("{{ page.images[#{@array_index}].alt }}").render(context)
      @caption = Liquid::Template.parse("{{ page.images[#{@array_index}].caption | markdownify }}").render(context) if @caption
...
Liquid::Template.register_tag('inline_image', Jekyll::FigureImageTag)
