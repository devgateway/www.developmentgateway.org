...
(function(window, document, undefined) {
  ...
  var height_fixed_header = 0, // land below header with position:fixed
  ...
  smoothScrollTo.el = function(element, progressCb, doneCb) {
    var scrollTop = getScrollTopElement(element),
        cutPaddingNotDisabled = element.getAttribute('data-smoothscroll-ignore-padding') === null;

    if (cutPaddingNotDisabled) {
      var computedStyle = window.getComputedStyle(element, null),
          cssPaddingTop = computedStyle.getPropertyValue('padding-top'),
          paddingTop = parseInt(cssPaddingTop, 10);  // "240px" => 240
      scrollTop += (paddingTop / 2);
    }

    return smoothScrollTo(scrollTop, doneCb, progressCb);
  };
  ...
  // hooks for changing the header offset and scroll period
  smoothScrollTo.setHeader = function(to) {
    if (typeof to !== 'number') {
      console.error('setHeader requires a number (of pixels) as its only argument. Got', to);
      return;
    }
    height_fixed_header = to;
  };
  smoothScrollTo.setDuration = function(to) {
    if (typeof to !== 'number') {
      console.error('setDuration requires a number (of milliseconds) as its only argument. Got', to);
      return;
    }
    period = to;
  };
